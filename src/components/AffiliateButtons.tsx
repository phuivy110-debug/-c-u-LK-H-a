...
export const AffiliateButtons: React.FC<AffiliateButtonsProps> = ({
  ...
  const validTikTok = validateAffiliateUrl(tiktokUrl, 'tiktok');

  if (!validShopee && !validTikTok) {
    return (
      <div className="w-full bg-slate-100 text-slate-500 py-2 px-3 rounded-xl text-[11px] sm:text-xs font-semibold text-center border border-slate-200">
        Đang cập nhật link mua hàng
      </div>
    );
  }
      ...
      <div className="grid grid-cols-2 gap-1.5 pt-0.5">
        {validShopee ? (
          <a
            ...
            title={`Mua ${productName} trên Shopee`}
          >
        ...
        {validTikTok ? (
          <a
            ...
            title={`Mua ${productName} trên TikTok Shop`}
          >
